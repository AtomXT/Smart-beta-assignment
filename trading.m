function earning = trading_date(start_date,end_date,freq)
%start_date 开始交易的日期
%end_date 交易结束的日期
%freq 交易间隔
...
Assets = 1000000 - 0.003*100*old_prices(start_date,:)*number';%总资产是减少手续费部分
...
%分别计算单次收益
    for i = 1:10
        if flag1(i) == 1
           temp_profit0(i) = (old_prices(sell(end),i) - old_prices(buy(2,i),i)).*100.*number(i).*0.997;
        else
           temp_profit0(i) = 0;       
        end
    end
...
%    profit = [profit, temp_profit];%储存本次盈利
    ...
    if (date<=end_date-freq)%判断是否到达交易结束日
            ...
            buy(2,:) = date*ones(1,10).*flag1 + buy(1,:).*(1-flag1);%记录买入点
            new_number = floor(Total./n./(100*1.003*new_prices(date,:)));%判断每只股票可以买入多少手，向下取整
